...
import { Menu, MenuItem, protocol, nativeImage as nativeImage2, app } from "electron";

// src/browser/popup.ts
import { EventEmitter } from "node:events";
import { BrowserWindow as BrowserWindow2, screen } from "electron";
...
var getIconImage = (extension) => {
  ...
  return iconAbsolutePath ? nativeImage.createFromPath(iconAbsolutePath) : void 0;
};
...
var getAllWindows = () => [...BaseWindow.getAllWindows(), ...BrowserWindow.getAllWindows()];

// src/browser/popup.ts
import debug from "debug";
var d = debug("electron-chrome-extensions:popup");
var supportsPreferredSize = () => {
  const major = parseInt(process.versions.electron.split(".").shift() || "", 10);
  return major >= 12;
};
var _PopupView = class _PopupView extends EventEmitter {
  constructor(opts) {
    super();
...
var d2 = debug2("electron-chrome-extensions:browserAction");
if (!app.isReady()) {
...
var BrowserActionAPI = class {
        ...
        case "extension-icon": {
          ...
          if (extension && iconDetails) {
            if (typeof iconDetails.path === "string") {
              ...
              if (iconAbsPath) iconImage = nativeImage2.createFromPath(iconAbsPath);
              ...
              iconImage = nativeImage2.createFromDataURL(iconDetails.imageData);
            } else if (typeof iconDetails.imageData === "object") {
              const imageData = matchSize(iconDetails.imageData, imageSize, resizeType);
              iconImage = imageData ? nativeImage2.createFromDataURL(imageData) : void 0;
            }
  ...
  activateContextMenu(details) {
    ...
    const manifest = getExtensionManifest(extension);
    const menu = new Menu();
    const append = (opts) => menu.append(new MenuItem(opts));
...
var d5 = debug5("electron-chrome-extensions:webNavigation");
...
import { Menu as Menu2, MenuItem as MenuItem2 } from "electron";
...
// src/browser/api/lib/native-messaging-host.ts
var d7 = debug7("electron-chrome-extensions:nativeMessaging");
  ...
  switch (process.platform) {
    case "darwin":
      searchPaths = [
        ...
        path2.join(app2.getPath("userData"), "NativeMessagingHosts", appJson)
      ];
      break;
    case "linux":
      searchPaths = [
...
var NativeMessagingHost = class {
  ...
  async launch(application, extensionId) {
    ...
    let isFile = false;
    try {
      const stat = await fs2.stat(config.path);
      isFile = stat.isFile();
    } catch (error) {
    ...
    this.process = spawn2(config.path, [extensionUrl], {
      shell: false
    });
...
var RuntimeAPI = class extends EventEmitter3 {
  ...
  getInstallStatePath() {
    ...
    return storagePath ? join3(storagePath, "electron-chrome-extensions", "extension-versions.json") : void 0;
  }
  readInstallState() {
...
import { app as app3, Notification } from "electron";
var getBody = (opts) => {
...
var resolvePartitionImpl = (partition) => session.fromPartition(partition);
...
var d8 = debug8("electron-chrome-extensions:router");
...
var ExtensionRouter = class {
  constructor(session2, delegate = RoutingDelegate.get()) {
    ...
    app4.on("web-contents-created", (event, webContents2) => {
...
var UserScriptsAPI = class {
  ...
  getStorageFilePath() {
    const storagePath = this.ctx.session.getStoragePath();
    return storagePath ? join5(storagePath, "electron-chrome-extensions", "user-scripts.json") : void 0;
  }
  readPersistedScripts() {
    const filePath = this.getStorageFilePath();
    if (!filePath || !existsSync2(filePath)) return { version: 1, extensions: {} };
    try {
  ...
  persistExtension(extensionId) {
    ...
    try {
      ...
      writeFileSync2(filePath, JSON.stringify(persisted), "utf8");
    } catch (error) {
  ...
  installDocumentStartHandler() {
    ...
    ipcMain2.handle(sendMessageChannel, (event, details) => {
      const api = instances.get(event.sender.session);
      return api?.sendUserScriptMessage(event, details) ?? void 0;
    });
    ipcMain2.on(connectChannel, (event, details) => {
      const api = instances.get(event.sender.session);
      api?.connectUserScriptPort(event, details);
    });
    ipcMain2.on(responseChannel, (event, details) => {
      const api = instances.get(event.sender.session);
      api?.resolveUserScriptMessage(details);
    });
    ipcMain2.on(portMessageChannel, (event, details) => {
      const api = instances.get(event.sender.session);
      api?.forwardUserScriptPortMessage(event, details);
    });
    ipcMain2.on(portDisconnectChannel, (event, details) => {
      const api = instances.get(event.sender.session);
...
var ElectronChromeExtensions = class _ElectronChromeExtensions extends EventEmitter4 {
  constructor(opts) {
    ...
    const { license, session: session2 = electronSession.defaultSession, ...impl } = opts || {};
    checkVersion();
