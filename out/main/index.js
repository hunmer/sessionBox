...
const electronApp = {
  setAppUserModelId(id2) {
    if (platform.isWindows)
      require$$1.app.setAppUserModelId(is.dev ? process.execPath : id2);
  },
  setAutoLaunch(auto) {
    if (platform.isLinux)
      return false;
    const isOpenAtLogin = () => {
      return require$$1.app.getLoginItemSettings().openAtLogin;
    };
    if (isOpenAtLogin() !== auto) {
      require$$1.app.setLoginItemSettings({ openAtLogin: auto });
  ...
  skipProxy() {
    return require$$1.session.defaultSession.setProxy({ mode: "direct" });
  }
};
const optimizer = {
  ...
  registerFramelessWindowIpc() {
    require$$1.ipcMain.on("win:invoke", (event, action) => {
      const win = require$$1.BrowserWindow.fromWebContents(event.sender);
      if (win) {
        if (action === "show") {
          win.show();
...
function updateGroup(id2, data) {
  const groups = getCollection("groups");
  const idx = groups.findIndex((g) => g.id === id2);
  if (idx === -1) throw new Error(`分组 ${id2} 不存在`);
...
function reorderGroups(groupIds) {
  const groups = getCollection("groups");
  groupIds.forEach((id2, order) => {
    const g = groups.find((g2) => g2.id === id2);
    if (g) g.order = order;
  });
  setCollection("groups", groups);
}
...
function createAccount(data) {
  const accounts = getCollection("accounts");
  const account = { ...data, id: require$$0.randomUUID() };
  accounts.push(account);
  setCollection("accounts", accounts);
  return account;
}
function updateAccount(id2, data) {
  const accounts = getCollection("accounts");
  const idx = accounts.findIndex((a) => a.id === id2);
  if (idx === -1) throw new Error(`账号 ${id2} 不存在`);
  accounts[idx] = { ...accounts[idx], ...data };
  setCollection("accounts", accounts);
}
function deleteAccount(id2) {
...
function createProxy(data) {
  ...
  const proxy = { ...data, id: require$$0.randomUUID() };
  proxies.push(proxy);
  setCollection("proxies", proxies);
  return proxy;
}
function updateProxy(id2, data) {
  const proxies = getCollection("proxies");
  const idx = proxies.findIndex((p) => p.id === id2);
  if (idx === -1) throw new Error(`代理 ${id2} 不存在`);
  proxies[idx] = { ...proxies[idx], ...data };
  setCollection("proxies", proxies);
}
function deleteProxy(id2) {
  const accounts = getCollection("accounts").map(
    (a) => a.proxyId === id2 ? { ...a, proxyId: void 0 } : a
  );
  setCollection("accounts", accounts);
  const groups = getCollection("groups").map(
    (g) => g.proxyId === id2 ? { ...g, proxyId: void 0 } : g
  );
  setCollection("groups", groups);
...
function createTab(data) {
  ...
  const tab = { ...data, id: require$$0.randomUUID() };
  tabs.push(tab);
  setCollection("tabs", tabs);
  return tab;
}
function updateTab(id2, data) {
  const tabs = getCollection("tabs");
  const idx = tabs.findIndex((t) => t.id === id2);
  if (idx === -1) return;
  tabs[idx] = { ...tabs[idx], ...data };
  setCollection("tabs", tabs);
}
function deleteTab(id2) {
  const tabs = getCollection("tabs").filter((t) => t.id !== id2);
  setCollection("tabs", tabs);
}
function reorderTabs(tabIds) {
  const tabs = getCollection("tabs");
  tabIds.forEach((id2, order) => {
    const t = tabs.find((t2) => t2.id === id2);
    if (t) t.order = order;
  });
  setCollection("tabs", tabs);
}
...
function createFavoriteSite(data) {
  ...
  const site = { ...data, id: require$$0.randomUUID() };
  sites.push(site);
...
function requireBrowser() {
  ...
  (function(module2, exports$1) {
    ...
    module2.exports = requireCommon$1()(exports$1);
...
function useAutoUpdater() {
  ...
  mainExports.autoUpdater.on("error", (error2) => {
    console.error("[AutoUpdater] 更新错误:", error2);
    sendToRenderer("update:error", {
      message: error2.message || "检查更新失败"
    });
  });
  ...
  const checkForUpdates = async () => {
    try {
      ...
      console.error("[AutoUpdater] 检查更新失败:", error2);
      return {
        success: false,
        error: error2.message || "检查更新失败"
      };
    }
  };
  const downloadUpdate = async () => {
    try {
      ...
      console.error("[AutoUpdater] 下载更新失败:", error2);
      return {
        success: false,
        error: error2.message || "下载更新失败"
      };
    }
  };
  const quitAndInstall = (isSilent = false) => {
    mainExports.autoUpdater.quitAndInstall(isSilent, true);
  };
  ...
  const getUpdateInfo = () => {
    try {
      const latestYmlPath = require$$1$2.join(require$$1.app.getAppPath(), "latest.yml");
      const content = require$$1$1.readFileSync(latestYmlPath, "utf-8");
...
function registerUpdaterIpc() {
  ...
  require$$1.ipcMain.handle("updater:download", async () => {
    return await updater.downloadUpdate();
  });
  require$$1.ipcMain.handle("updater:install", async (_event, isSilent = false) => {
    updater.quitAndInstall(isSilent);
    return { success: true };
  });
  require$$1.ipcMain.handle("updater:get-version", () => {
    return {
      success: true,
      version: updater.getCurrentVersion()
    };
  });
  require$$1.ipcMain.handle("updater:get-info", () => {
...
function registerIpcHandlers() {
  ...
  require$$1.ipcMain.handle("group:delete", (_e, id2) => deleteGroup(id2));
  require$$1.ipcMain.handle("group:reorder", (_e, groupIds) => reorderGroups(groupIds));
  require$$1.ipcMain.handle("account:list", () => listAccounts());
  require$$1.ipcMain.handle("account:create", (_e, data) => createAccount(data));
  require$$1.ipcMain.handle(
    "account:update",
  ...
  require$$1.ipcMain.handle("account:delete", (_e, id2) => {
    ...
    if (account?.icon?.startsWith("img:")) {
      const filePath = require$$1$2.join(iconDir, account.icon.slice(4));
  ...
  require$$1.ipcMain.handle("account:createDesktopShortcut", (_e, accountId) => {
    ...
    const shortcutPath = require$$1$2.join(desktopPath, `${account.name}.url`);
    ...
    if (account.icon?.startsWith("img:")) {
      ...
      const imgPath = require$$1$2.join(iconDir, imgName);
      if (node_fs.existsSync(imgPath) && !imgName.endsWith(".svg")) {
        const icoName = imgName.replace(/\.[^.]+$/, ".ico");
        const icoPath = require$$1$2.join(iconDir, icoName);
...
$img.Dispose()`;
            ...
            require$$1$6.execSync(`powershell -NoProfile -EncodedCommand ${encoded}`, {
              windowsHide: true,
              timeout: 5e3
            });
          } catch {
  ...
  require$$1.ipcMain.handle("account:uploadIcon", async () => {
    ...
    const destPath = require$$1$2.join(iconDir, fileName);
...
if (!gotTheLock) {
  ...
  let createWindow = function() {
    ...
    const mainWindow2 = new require$$1.BrowserWindow({
      ...
      webPreferences: {
        preload: require$$1$2.join(__dirname, "../preload/index.js"),
        sandbox: false
      }
    });
    ...
    if (process.env.ELECTRON_RENDERER_URL) {
      ...
      mainWindow2.loadFile(require$$1$2.join(__dirname, "../renderer/index.html"));
    }
  };
