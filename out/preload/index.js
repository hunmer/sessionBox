...
const api = {
    ...
    settings: {
        ...
        getRestoreLastUrl: () => ipcRenderer.invoke('settings:getRestoreLastUrl'),
        setRestoreLastUrl: (enabled) => ipcRenderer.invoke('settings:setRestoreLastUrl', enabled),
    ...
    theme: {
        ...
        setNativeTheme: (source) => ipcRenderer.invoke('theme:setNativeTheme', source),
    ...
    siteData: {
        getInfo: (tabId) => ipcRenderer.invoke('siteData:getInfo', tabId),
        clear: (tabId) => ipcRenderer.invoke('siteData:clear', tabId),
        importCookies: (tabId, cookieText) => ipcRenderer.invoke('siteData:importCookies', tabId, cookieText),
    },
    sniffer: {
    ...
    download: {
        ...
        listSystem: () => ipcRenderer.invoke('download:listSystem'),
        removeSystem: (gid) => ipcRenderer.invoke('download:removeSystem', gid),
        clearSystemFinished: () => ipcRenderer.invoke('download:clearSystemFinished'),
        clearSystemAll: () => ipcRenderer.invoke('download:clearSystemAll'),
    ...
    // Workflow agent 工具执行
    agent: {
        execTool: (toolType, params, targetTabId) => ipcRenderer.invoke('agent:execTool', toolType, params, targetTabId),
    },
    // Skill 管理
    skill: {
    ...
    debugger: {
        createWindow: () => ipcRenderer.invoke('debugger:create-window'),
    },
    // 主进程 → 渲染进程事件监听
    on: (event, callback) => {
