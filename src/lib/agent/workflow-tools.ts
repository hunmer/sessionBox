 ...
 *
 * 提供工作流编辑专用的 9 个 Anthropic 工具 schema，
...
export const WORKFLOW_TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    name: 'get_workflow',
    description:
      '获取当前工作流的完整结构信息，包括所有节点和连线。默认返回精简摘要（仅节点 ID、类型、标签和连线关系），设置 summarize=false 可获取包含节点 data 在内的完整详情。',
    input_schema: {
      type: 'object',
      properties: {
        summarize: {
          type: 'boolean',
          description: '是否返回精简摘要。默认 true，仅返回节点 ID/类型/标签和连线；设为 false 返回完整 data。',
          default: true,
        },
      },
    },
  ...
  },
  {
    name: 'update_node',
    description:
      '更新指定节点的参数数据。只需传入要修改的字段，未传入的字段保持不变。更新前建议先用 get_workflow(summarize=false) 查看节点当前完整数据。',
    input_schema: {
      type: 'object',
      properties: {
        nodeId: {
...
| 操作 | 工具名 | 说明 |
|------|--------|------|
| 查看工作流 | get_workflow | 获取当前工作流的所有节点和连线 |
...
1. **先了解现状**：编辑前先用 get_workflow 查看当前工作流结构，了解现有节点和连线关系。
