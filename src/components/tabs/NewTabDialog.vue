...
// 点击容器
function handleSelectContainer(container: Container) {
  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="w-[80vw] sm:max-w-[640px] p-4 gap-3" show-close-button>
      ...
      <div v-if="containers.length > 0">
        <div class="text-xs text-muted-foreground mb-1.5 px-1">容器</div>
        <div class="flex gap-2 overflow-x-auto pb-1">
          <button
            ...
            @click="handleSelectContainer(container)"
          >
            <span class="w-10 h-10 rounded-lg bg-muted flex items-center justify-center overflow-hidden text-lg">
              ...
              />
              <span v-else class="leading-none">{{ container.icon }}</span>
            </span>
            <span class="text-[11px] text-center leading-tight truncate w-full">{{ container.name }}</span>
          </button>
        </div>
      </div>
      ...
      <!-- 无结果提示 -->
      <div
        v-if="containers.length === 0 && filteredHistory.length === 0 && !urlInput.trim()"
